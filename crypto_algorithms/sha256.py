def right_rotate(value, bits):
    return ((value >> bits) | (value << (32 - bits))) & 0xFFFFFFFF


def text_to_bytes(message):
    if isinstance(message, bytes):
        return list(message)

    return list(str(message).encode("utf-8"))

def int_to_8_bytes(number):
    result = []

    for shift in range(56, -1, -8):
        result.append((number >> shift) & 0xFF)

    return result


def four_bytes_to_int(b0, b1, b2, b3):
    return (
        (b0 << 24)
        | (b1 << 16)
        | (b2 << 8)
        | b3
    )


def int_to_hex_8_digits(number):
    hex_chars = "0123456789abcdef"
    result = ""

    for shift in range(28, -1, -4):
        value = (number >> shift) & 0xF
        result += hex_chars[value]

    return result


def sha256(message):
    message_bytes = text_to_bytes(message)

    h0 = 0x6a09e667
    h1 = 0xbb67ae85
    h2 = 0x3c6ef372
    h3 = 0xa54ff53a
    h4 = 0x510e527f
    h5 = 0x9b05688c
    h6 = 0x1f83d9ab
    h7 = 0x5be0cd19

    K = [
        0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
        0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
        0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
        0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
        0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
        0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
        0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
        0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
        0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
        0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
        0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
        0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
        0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5,
        0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
        0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
        0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
    ]

    original_bit_length = len(message_bytes) * 8

    message_bytes.append(0x80)

    while (len(message_bytes) * 8) % 512 != 448:
        message_bytes.append(0)

    length_bytes = int_to_8_bytes(original_bit_length)

    for byte in length_bytes:
        message_bytes.append(byte)

    for block_start in range(0, len(message_bytes), 64):
        block = message_bytes[block_start:block_start + 64]

        words = []

        for i in range(0, 64, 4):
            word = four_bytes_to_int(
                block[i],
                block[i + 1],
                block[i + 2],
                block[i + 3]
            )
            words.append(word)

        for i in range(16, 64):
            s0 = (
                right_rotate(words[i - 15], 7)
                ^ right_rotate(words[i - 15], 18)
                ^ (words[i - 15] >> 3)
            )

            s1 = (
                right_rotate(words[i - 2], 17)
                ^ right_rotate(words[i - 2], 19)
                ^ (words[i - 2] >> 10)
            )

            new_word = (
                words[i - 16]
                + s0
                + words[i - 7]
                + s1
            ) & 0xFFFFFFFF

            words.append(new_word)

        a = h0
        b = h1
        c = h2
        d = h3
        e = h4
        f = h5
        g = h6
        h = h7

        for i in range(64):
            S1 = (
                right_rotate(e, 6)
                ^ right_rotate(e, 11)
                ^ right_rotate(e, 25)
            )

            choice = (e & f) ^ ((~e) & g)

            temp1 = (
                h
                + S1
                + choice
                + K[i]
                + words[i]
            ) & 0xFFFFFFFF

            S0 = (
                right_rotate(a, 2)
                ^ right_rotate(a, 13)
                ^ right_rotate(a, 22)
            )

            majority = (a & b) ^ (a & c) ^ (b & c)

            temp2 = (S0 + majority) & 0xFFFFFFFF

            h = g
            g = f
            f = e
            e = (d + temp1) & 0xFFFFFFFF
            d = c
            c = b
            b = a
            a = (temp1 + temp2) & 0xFFFFFFFF

        h0 = (h0 + a) & 0xFFFFFFFF
        h1 = (h1 + b) & 0xFFFFFFFF
        h2 = (h2 + c) & 0xFFFFFFFF
        h3 = (h3 + d) & 0xFFFFFFFF
        h4 = (h4 + e) & 0xFFFFFFFF
        h5 = (h5 + f) & 0xFFFFFFFF
        h6 = (h6 + g) & 0xFFFFFFFF
        h7 = (h7 + h) & 0xFFFFFFFF

    final_hash = (
        int_to_hex_8_digits(h0)
        + int_to_hex_8_digits(h1)
        + int_to_hex_8_digits(h2)
        + int_to_hex_8_digits(h3)
        + int_to_hex_8_digits(h4)
        + int_to_hex_8_digits(h5)
        + int_to_hex_8_digits(h6)
        + int_to_hex_8_digits(h7)
    )

    return final_hash


if __name__ == "__main__":
    user_input = input("Enter text to hash with SHA-256: ")
    hashed_value = sha256(user_input)

    print("SHA-256 hash:")
    print(hashed_value)